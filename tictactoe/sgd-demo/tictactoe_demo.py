...
class RecordingLinear(nn.Module):
    def __init__(self, input_dimensions, output_dimensions, name):
        super().__init__()
...
def train():
    ...
    for sample_index in range(3000):
        ...
        start_parameters = [x.detach().numpy().tolist() for x in model.parameters()]
        optimizer.step()
        adjusted_parameters = [x.detach().numpy().tolist() for x in model.parameters()]

        # Print the loss
        if sample_index % 1 == 0:
            print(sample_index, ' Loss: ', loss.item())

        # print(json.dumps([x.grad.detach().numpy().tolist() for x in model.parameters()]))

        log.append({
            'timestamp': now(),
            'type': 'sample_processed',
            'actual_outputs': actual_outputs.detach().numpy().tolist(),
            'start_parameters': start_parameters,
            'adjusted_parameters': adjusted_parameters,
            'loss': loss.detach().numpy().tolist(),
            'gradients': [x.grad.detach().numpy().tolist() for x in model.parameters()],
        })
